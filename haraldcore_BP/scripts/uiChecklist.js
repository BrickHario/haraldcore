...
function taskIsDone(todo, taskName) {
  ...
  try {
    return todo.hasParticipant(`§a✔ ${taskName}`);
  } catch (_) {
    return false;
  }
}
...
function buildTaskText() {
  ...
  for (const task of TASKS) {
    const done = taskIsDone(todo, task);
    if (done) doneCount++;
    lines.push(done ? `§a✔ ${task}` : `§7☐ ${task}`);
  }
...
export function isChecklistOpen(player) {
  return openChecklistPlayers.has(player.id);
}

export async function showHaraldChecklist(player) {
  if (!player || isChecklistOpen(player)) return;

  openChecklistPlayers.add(player.id);
...
const form = new ActionFormData()
  ...
  .label("§eTip: Finding chests might help..");
    ...
    openChecklistPlayers.delete(player.id);
  }
}
...
export function registerChecklistUI() {
  system.beforeEvents.startup.subscribe((event) => {
    event.customCommandRegistry.registerCommand(
      {
        name: "harald:tasks",
        description: "Open the HaraldCore task checklist",
